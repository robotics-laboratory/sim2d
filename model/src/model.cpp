...
Model::Model(const std::string& config_path) : params_(config_path) {
...

    {
        ...
        // new loading data just here
        const auto node = YAML::LoadFile(config_path)["tf_static"];
        for (const auto& tf : node) {
            TfFrame frame;
            frame.frame_id = tf["frame_id"].as<std::string>();
            frame.child_frame_id = tf["child_frame_id"].as<std::string>();
            tf2::Vector3 t(
                tf["translation"]["x"].as<double>(),
                tf["translation"]["y"].as<double>(),
                tf["translation"]["z"].as<double>());
            tf2::Quaternion q(
                tf["rotation"]["x"].as<double>(),
                tf["rotation"]["y"].as<double>(),
                tf["rotation"]["z"].as<double>(),
                tf["rotation"]["w"].as<double>());
            frame.transform.setOrigin(t);
            frame.transform.setRotation(q);
            cache_.tf_frames.push_back(frame);
        }
...
tf2::Transform Model::getLatestTranform(
    ...
    // }
    for (const auto& frame : cache_.tf_frames) {
        if (frame.frame_id == source && frame.child_frame_id == target) {
            return frame.transform;
        }
    }
    // обратное направление
    for (const auto& frame : cache_.tf_frames) {
        if (frame.frame_id == target && frame.child_frame_id == source) {
            return frame.transform.inverse();
        }
    }
    throw common::Exception() << "Transform not found: " << source << " -> " << target;
